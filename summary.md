...
*   **年龄还原**：由于在训练阶段模型是以归一化的差值预测年龄的，所以前向推理结束后必须进行反归一化。
    ...
    * **推理模式与评估模式在公式执行上完全一致。** 关于公式内的具体系数：
        ...
        * **在推理时**，它们被保存在预训练模型的检查点（checkpoint 的 `state_dict` 或配置）中并随权重一起加载。不同的预训练模型具有不同的值。例如，在官方发布的最新 `mivolo_v2` (分辨率为 384x384) 模型配置文件中，这三个数值确切地为：**`min_age: 0`，`max_age: 122`，`avg_age: 61.0`**。
        * **如何读取与下载384权重**：你可以通过极简的 Python 代码从官方权重文件中读取出它们。官方最新的 384 分辨率版本（即 `mivolov2_d1_384x384`）托管在 HuggingFace，页面地址为 `https://huggingface.co/iitolstykh/mivolo_v2`。由于它是 HuggingFace 的标准格式，你可以使用 `huggingface_hub` 读取参数：
          ```python
          from huggingface_hub import hf_hub_download
          import json

          # 下载官方 mivolo_v2_d1_384 配置文件
          config_path = hf_hub_download(repo_id="iitolstykh/mivolo_v2", filename="config.json")
          with open(config_path) as f:
              config = json.load(f)
          print("min_age:", config.get("min_age"))
          print("max_age:", config.get("max_age"))
          print("avg_age:", config.get("avg_age"))

          # (注：如果下载的是早期的 v1 `.pth.tar` 格式权重，使用 `torch.load()` 加载即可提取)
          ```
